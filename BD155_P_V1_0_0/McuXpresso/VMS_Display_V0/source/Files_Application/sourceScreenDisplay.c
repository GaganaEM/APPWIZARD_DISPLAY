...
uint8_t itemSelected[NUM_ITEMS] = {0};  // Selection state for each item (0 = not selected, 1 = selected)
uint8_t buttonState = 0;            // Current button state (0 = not pressed, 1 = pressed)
uint8_t prevButtonState = 0;        // Previous button state
uint8_t prevButtonState_editing = 0;
const int maxScroll = NUM_ITEMS - 1;  // Maximum scroll position
uint8_t selected_flag;
int32_t mCurPosValuescreen = 0;
int32_t prevPosValuescreen = 0;
...
void ProcessScreen06(void)
{
	...
	mCurPosValuescreen = ENC_GetPositionValue(DEMO_ENC_BASEADDR);
	debounce_encoder();
	if ( abs(mCurPosValuescreen - prevPosValuescreen) > ENCODER_THRESHOLD)
	{
	   //buttonState = GPIO_ReadPinInput(GPIO1, 11);

	if (mCurPosValuescreen > prevPosValuescreen)
	{
	...
	prevPosValuescreen = mCurPosValuescreen;
   }
...
void ProcessScreen10(void)
{
	...
	if ( abs(mCurPosValuescreen - prevPosValuescreen) > ENCODER_THRESHOLD)
	{
		...
		{
			//counter++ ;
			if (giScreenSwitch == 10 )//&& counter==3)
			{
				//counter = 0;
			    giScreenSwitch = 6;
			}
...
void debounce_encoder() {
    ...
    uint32_t currentTime = millis(); // Use a platform-specific time function
    if (currentTime - lastUpdateTime < 50)
    { // Debounce for 50ms
        return;
    }
    lastUpdateTime = currentTime;
    // Proceed with encoder logic
}
/* User Setting Screen */
void ProcessScreen13(void)
{
	...
	if (selected_flag == 0) // Scrolling mode
	{
	    debounce_encoder();
	    mCurPosValue = ENC_GetPositionValue(DEMO_ENC_BASEADDR);
	    buttonState = GPIO_ReadPinInput(GPIO1, 11);

	    // Encoder Scroll Logic
	    if (mCurPosValue > prevPosValue)
	    {
	        gcUserScroll++;
	        if (gcUserScroll > maxScroll)
	        {
	            gcUserScroll = maxScroll; // Limit to maxScroll
	        }
	    } else if (mCurPosValue < prevPosValue) {
	        gcUserScroll--;
	        if (gcUserScroll < 0) {
	            gcUserScroll = 0; // Limit to minimum scroll position
	        }
	    }
	    prevPosValue = mCurPosValue;

	    // Button Press Logic
	    if (buttonState == 1 && prevButtonState == 0 && buttonPressed == 0)
	    {
	        buttonPressed = 1; // Mark button as pressed
	        itemSelected[gcUserScroll] = !itemSelected[gcUserScroll];
	        selected_flag = 1; // Enter editing mode
	        mCurPosValue = 0;
	        prevPosValue = 0;
	    }
	    else if (buttonState == 0)
	    {
	        buttonPressed = 0; // Reset button press when released
	    }

	    prevButtonState = buttonState;
	}

	if (itemSelected[gcUserScroll] == 1 && selected_flag == 1) // Editing mode
	{
	    debounce_encoder();
	    mCurPosValue = ENC_GetPositionValue(DEMO_ENC_BASEADDR);
	    buttonState = GPIO_ReadPinInput(GPIO1, 11);
	    ...
	    {
	        buttonPressed = 1; // Mark button as pressed
	        itemSelected[gcUserScroll] = 0;
	        selected_flag = 0; // Exit editing mode
	    }
	    else if (buttonState == 0)
	    {
	        buttonPressed = 0; // Reset button press when released
	    ...
	    if(gcUserScroll == 0)
	{
		if (abs(mCurPosValue - prevPosValue) > ENCODER_THRESHOLD)
		{
			if (mCurPosValue > prevPosValue)
			{
				if(giMode == 0)
